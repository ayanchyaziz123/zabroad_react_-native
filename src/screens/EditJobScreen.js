  ...
  async function handleSave() {
    ...
    try {
      ...
      if (image) {
        ...
        fd.append('image', { uri: image.uri, name: `job.${ext}`, type: mime });
      }
      await api(`/jobs/${job.id}/`, { method: 'PATCH', body: fd });
      onSave && onSave({ ...job, title: title.trim(), company: company.trim(), location: location.trim(), desc: desc.trim(), category, image_url: image ? null : job.image_url });
      navigation.goBack();
    } catch (e) {
        ...
        <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" contentContainerStyle={s.form}>
          ...
          <View style={s.fieldGroup}>
            ...
            <View style={s.inputRow}>
              ...
              <TextInput style={s.input} placeholder="Job title" placeholderTextColor={C.c35} value={title} onChangeText={setTitle} />
            </View>
          </View>
          ...
          <View style={s.fieldGroup}>
            ...
            <View style={s.inputRow}>
              ...
              <TextInput style={s.input} placeholder="Company name" placeholderTextColor={C.c35} value={company} onChangeText={setCompany} />
            </View>
          </View>
            ...
            <View style={s.catGrid}>
              {CATEGORY_OPTIONS.map(cat => {
                ...
                return (
                  <TouchableOpacity
                    key={cat.key}
                    style={[s.catOption, active && { borderColor: C.green, backgroundColor: C.greenD }]}
                    onPress={() => setCategory(cat.key)}
                    activeOpacity={0.8}
                  >
                    <Ionicons name={cat.icon} size={14} color={active ? C.green : C.c35} />
                    <Text style={[s.catOptionTxt, { color: active ? C.green : C.c35 }]}>{cat.label}</Text>
                  </TouchableOpacity>
                );
              })}
            </View>
          ...
          <View style={s.fieldGroup}>
            ...
            <TextInput
              style={s.textArea}
              placeholder="Describe the role, hours, pay…"
          ...
          <View style={s.fieldGroup}>
            ...
            <TouchableOpacity style={s.imagePicker} onPress={pickImage} activeOpacity={0.8}>
              {image ? (
                ...
                <Image source={{ uri: job.image_url }} style={s.imagePreview} resizeMode="cover" />
              ) : (
...
const getStyles = (C) => StyleSheet.create({
  ...
  saveBtn:    { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 10, backgroundColor: C.green, borderRadius: 16, paddingVertical: 15 },
