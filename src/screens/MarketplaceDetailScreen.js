...
export default function MarketplaceDetailScreen({ route, navigation }) {
  ...
  const posterInitials = (item.poster || item.poster_name || '?')
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 110 }}>
        ...
        <View style={[s.mainCard, { backgroundColor: C.card, borderColor: C.border }]}>
          ...
          <View style={s.sellerRow}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={[s.sellerName, { color: C.cream }]}>{item.poster || item.poster_name || 'Unknown'}</Text>
      ...
      <View style={[s.bottomBar, { backgroundColor: C.nav, borderTopColor: C.border }]}>
        {isOwn ? (
          ...
          <>
            ...
            <TouchableOpacity
              style={[s.msgBtn, { backgroundColor: messaged ? C.greenD : ACCENT, borderColor: messaged ? C.green + '44' : 'transparent' }]}
              onPress={onMessage}
              activeOpacity={0.85}
            >
