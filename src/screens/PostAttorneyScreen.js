...
export default function PostAttorneyScreen({ navigation }) {
  const { colors: C } = useTheme();
  const s = useMemo(() => getStyles(C), [C]);
  const { user }      = useUser();
  const addAttorney   = useAttorneyStore(s => s.addAttorney);
  ...
  const canPost = name.trim().length > 2 && location.trim().length > 1 && desc.trim().length >= 20;

  async function postListing() {
    await addAttorney({
      name, firm, location, languages, specialty, price, desc, plan,
  ...
  async function handlePay() {
    ...
    setPaying(true);
    try {
      await api('/attorneys/payment-intent/', { method: 'POST', body: { plan } });
  ...
  function resetAll() {
    setSubmitted(false); setName(''); setFirm(''); setLocation('');
    setLanguages(''); setSpecialty(''); setPrice(''); setDesc(''); setPlan('free');
  ...
  if (submitted) {
    ...
    const accent     = activePlan.color || C.purple;
    return (
      <SafeAreaView style={s.safe} edges={['top', 'bottom']}>
        <Animated.View style={[s.successWrap, { opacity: successOpacity, transform: [{ scale: successScale }] }]}>
          <View style={[s.successCard, { backgroundColor: C.card, borderColor: accent + '44' }]}>
            <View style={[s.successIcon, { backgroundColor: accent + '22' }]}>
              <Ionicons name="scale" size={36} color={accent} />
            </View>
            <Text style={s.successTitle}>Listing Posted!</Text>
            <Text style={s.successName}>{name}</Text>
            <Text style={s.successSub}>{firm ? `${firm} · ` : ''}{location}</Text>
            <View style={[s.successPlanBadge, { backgroundColor: accent + '22', borderColor: accent + '55' }]}>
              <Ionicons name="checkmark-circle" size={13} color={accent} />
              <Text style={[s.successPlanTxt, { color: accent }]}>{activePlan.label} Plan · {activePlan.price}</Text>
            </View>
            <Text style={s.successNote}>Your profile is now visible to your community.</Text>
            <TouchableOpacity style={[s.doneBtn, { backgroundColor: C.purple }]} onPress={() => navigation.navigate('Attorney')} activeOpacity={0.85}>
        ...
        <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" contentContainerStyle={s.form}>
          ...
          <View style={s.fieldGroup}>
            ...
            <View style={s.inputRow}>
              <Ionicons name="person-outline" size={16} color={C.c35} />
              <TextInput style={s.input} placeholder="e.g. Sarah Kim, Esq." placeholderTextColor={C.c35} value={name} onChangeText={setName} />
            </View>
          </View>

          {/* Firm */}
          <View style={s.fieldGroup}>
            <Text style={s.fieldLabel}>FIRM / PRACTICE NAME</Text>
            <View style={s.inputRow}>
              <Ionicons name="business-outline" size={16} color={C.c35} />
              <TextInput style={s.input} placeholder="e.g. Kim Immigration Law" placeholderTextColor={C.c35} value={firm} onChangeText={setFirm} />
            </View>
          </View>
          ...
          <View style={s.fieldGroup}>
            ...
            <View style={s.inputRow}>
              ...
              <TextInput style={s.input} placeholder="e.g. Manhattan, NY" placeholderTextColor={C.c35} value={location} onChangeText={setLocation} />
            </View>
          </View>

          {/* Specialty */}
          <View style={s.fieldGroup}>
            <Text style={s.fieldLabel}>SPECIALTY / VISA TYPES</Text>
            <View style={s.inputRow}>
              <Ionicons name="scale-outline" size={16} color={C.c35} />
              <TextInput style={s.input} placeholder="e.g. H-1B, Asylum, Green Card" placeholderTextColor={C.c35} value={specialty} onChangeText={setSpecialty} />
            </View>
          </View>
          ...
          <View style={s.fieldGroup}>
            ...
            <View style={s.inputRow}>
              ...
              <TextInput style={s.input} placeholder="e.g. English, Spanish, Urdu" placeholderTextColor={C.c35} value={languages} onChangeText={setLanguages} />
            </View>
          </View>

          {/* Price */}
          <View style={s.fieldGroup}>
            <Text style={s.fieldLabel}>CONSULTATION / RATES</Text>
            <View style={s.inputRow}>
              <Ionicons name="cash-outline" size={16} color={C.c35} />
              <TextInput style={s.input} placeholder="e.g. Free consult · $200/hr" placeholderTextColor={C.c35} value={price} onChangeText={setPrice} />
            </View>
          </View>
          ...
          <View style={s.fieldGroup}>
            ...
            <TextInput
              style={s.textArea}
              placeholder="Describe your experience, how you help immigrants, and how to reach you…"
            ...
            />
            <Text style={[s.charCount, { color: desc.length >= 20 ? C.purple : C.c35 }]}>
          ...
          <View style={s.fieldGroup}>
            ...
            <View style={s.plansCol}>
              {PLANS.map(p => {
                ...
                const accent  = p.color || C.purple;
                const accentD = p.color ? p.color + '18' : C.purpleD;
                return (
                  <TouchableOpacity key={p.key} style={[s.planCard, active && { borderColor: accent, backgroundColor: accentD }]} onPress={() => setPlan(p.key)} activeOpacity={0.85}>
          ...
          <View style={[s.modalCard, { backgroundColor: C.card }]}>
            <View style={s.modalHeader}>
              <View>
                ...
                <Text style={s.modalSub}>{PLANS.find(p => p.key === plan)?.label} · {PLANS.find(p => p.key === plan)?.price} one-time</Text>
              </View>
            ...
            <TouchableOpacity style={[s.payBtn, { backgroundColor: C.purple }, paying && { opacity: 0.7 }]} onPress={handlePay} disabled={paying} activeOpacity={0.85}>
...
const getStyles = (C) => StyleSheet.create({
  ...
  postBtn:    { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, backgroundColor: C.purple, borderRadius: 16, paddingVertical: 15, shadowColor: C.purple, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.35, shadowRadius: 10, elevation: 6 },
  ...
  successName:     { fontSize: 16, fontWeight: '700', color: C.cream },
