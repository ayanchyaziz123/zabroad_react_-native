...
const CONSULT_MODES = [
  { key: 'in_person',  icon: 'business-outline',  label: 'In-Person Only',   sub: 'Patients visit your clinic'        },
  { key: 'telehealth', icon: 'desktop-outline',    label: 'Telehealth Only',  sub: 'Video / phone consultations'       },
  { key: 'both',       icon: 'git-merge-outline',  label: 'Both Options',     sub: 'Flexible for each patient'         },
];
...
function Field({ label, children, required, C }) {
  return (
    <View style={{ marginBottom: 16 }}>
      <Text style={{ fontSize: 11, fontWeight: '700', color: C.c35, letterSpacing: 1.2, textTransform: 'uppercase', marginBottom: 8 }}>
        {label}{required && <Text style={{ color: C.teal }}> *</Text>}
      </Text>
      {children}
    </View>
  );
...
export default function ListDoctorScreen({ navigation }) {
  ...
  function handleSubmit() {
    setSubmitted(true);
    Animated.parallel([
      Animated.spring(successOpacity, { toValue: 1, duration: 300, useNativeDriver: true }),
      ...
      <SafeAreaView style={s.safe} edges={['top', 'bottom']}>
        <Animated.View style={[s.successWrap, { opacity: successOpacity }]}>
          <Animated.View style={[{ alignItems: 'center' }, { transform: [{ scale: successScale }] }]}>
            <View style={[s.successIcon, { backgroundColor: C.tealD, borderColor: C.teal + '44' }]}>
              <Ionicons name="medical" size={40} color={C.teal} />
            </View>
            ...
            <View style={[s.successCard, { borderColor: C.teal + '44' }]}>
              ...
              {clinic     ? <Text style={s.previewRow}>{clinic} · {location}</Text>   : null}
              {specialties.length > 0 ? <Text style={s.previewRow}>{specialties.slice(0, 2).join(' · ')}</Text> : null}
              {languages.length > 0   ? <Text style={s.previewRow}>{languages.slice(0, 3).join(' · ')}</Text>   : null}
              {(() => {
                const p = SUBSCRIPTION_PLANS.find(pl => pl.key === selectedPlan);
                return p ? (
    ...
    <SafeAreaView style={s.safe} edges={['top', 'bottom']}>
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
        ...
        <View style={s.progressTrack}>
          <View style={[s.progressFill, { width: `${((step + 1) / STEPS.length) * 100}%`, backgroundColor: accent }]} />
        </View>
        ...
        >
          ...
          {step === 0 && (
            <View>
              ...
              <Field label="Clinic / Practice Name" required C={C}>
              ...
              <Field label="Years of Experience" C={C}>
          ...
          {step === 1 && (
            <View>
              ...
              <Field label="Specialties" required C={C}>
                <View style={s.chipGrid}>
                  {SPECIALTIES.map(sp => {
                    const sel = specialties.includes(sp);
                    return (
                      <TouchableOpacity key={sp} style={[s.chip, sel && { backgroundColor: accent + '22', borderColor: accent + '66' }]} onPress={() => toggle(specialties, setSpecialties, sp)}>
                        <Text style={[s.chipTxt, sel && { color: accent, fontWeight: '700' }]}>{sp}</Text>
                      </TouchableOpacity>
                    );
                  })}
                </View>
              </Field>
              <Field label="Languages Spoken" required C={C}>
                <View style={s.chipGrid}>
                  {LANGUAGES.map(lang => {
                    const sel = languages.includes(lang);
                    return (
                      <TouchableOpacity key={lang} style={[s.chip, sel && { backgroundColor: accent + '22', borderColor: accent + '66' }]} onPress={() => toggle(languages, setLanguages, lang)}>
                        <Text style={[s.chipTxt, sel && { color: accent, fontWeight: '700' }]}>{lang}</Text>
                      </TouchableOpacity>
                    );
                  })}
                </View>
              </Field>
              <Field label="Insurance & Payment Accepted" C={C}>
                <View style={s.chipGrid}>
                  {INSURANCE_OPTIONS.map(ins => {
                    const sel = insurances.includes(ins);
                    return (
                      <TouchableOpacity key={ins} style={[s.chip, sel && { backgroundColor: accent + '22', borderColor: accent + '66' }]} onPress={() => toggle(insurances, setInsurances, ins)}>
                        <Text style={[s.chipTxt, sel && { color: accent, fontWeight: '700' }]}>{ins}</Text>
                      </TouchableOpacity>
                    );
                  })}
                </View>
              </Field>
              <Field label="About Your Practice" required C={C}>
                <TextInput
                ...
                <Text style={{ fontSize: 11, color: bio.length >= 30 ? C.teal : C.c35, marginTop: 4 }}>
                  {bio.length}/500 {bio.length >= 30 ? '· Looks good' : `· ${30 - bio.length} more needed`}
                </Text>
              </Field>
            </View>
          )}
          ...
          {step === 2 && (
            <View>
              ...
              <Field label="Consultation Mode" required C={C}>
                {CONSULT_MODES.map(m => (
                  <TouchableOpacity
                    key={m.key}
                    style={[s.optionRow, consultMode === m.key && { borderColor: accent + '66', backgroundColor: accent + '11' }]}
                    onPress={() => setConsultMode(m.key)}
                    activeOpacity={0.8}
                  >
                    <View style={[s.optionIconWrap, { backgroundColor: consultMode === m.key ? accent + '22' : C.card2 }]}>
                      <Ionicons name={m.icon} size={18} color={consultMode === m.key ? accent : C.c35} />
                    </View>
                    <View style={{ flex: 1 }}>
                      <Text style={[s.optionLabel, consultMode === m.key && { color: accent }]}>{m.label}</Text>
                      <Text style={s.optionSub}>{m.sub}</Text>
                    </View>
                    <View style={[s.radio, consultMode === m.key && { borderColor: accent, backgroundColor: accent }]}>
                      {consultMode === m.key && <View style={s.radioDot} />}
                    </View>
                  </TouchableOpacity>
                ))}
              </Field>
              <Field label="Available Days" required C={C}>
                <View style={{ flexDirection: 'row', gap: 8, flexWrap: 'wrap' }}>
                  {AVAIL_DAYS.map(d => {
                    ...
                    return (
                      <TouchableOpacity
                        key={d}
                        style={[s.dayPill, sel && { backgroundColor: accent, borderColor: accent }]}
                        onPress={() => toggle(availDays, setAvailDays, d)}
                        activeOpacity={0.8}
                      >
              ...
              <Field label="Avg. Response Time" C={C}>
              ...
              <Field label="Accepting New Patients" C={C}>
                <View style={{ flexDirection: 'row', gap: 10 }}>
                  {[true, false].map(val => (
                    <TouchableOpacity
                      key={String(val)}
                      style={[s.chip, acceptsNew === val && { backgroundColor: accent + '22', borderColor: accent + '66' }]}
                      onPress={() => setAcceptsNew(val)}
                      activeOpacity={0.8}
                    >
                      <Text style={[s.chipTxt, acceptsNew === val && { color: accent, fontWeight: '700' }]}>
                        {val ? 'Yes, accepting' : 'Not currently'}
                      </Text>
                    </TouchableOpacity>
                  ))}
                </View>
          ...
          {step === 3 && (
            <View>
              ...
              <Text style={s.stepSub}>Upgrade anytime. All paid plans include a 7-day free trial.</Text>
              ...
              {SUBSCRIPTION_PLANS.map(plan => {
                ...
                return (
                  <TouchableOpacity
                    key={plan.key}
                    style={[s.planCard, active && { borderColor: plan.color + '99' }]}
                    onPress={() => setSelectedPlan(plan.key)}
                    activeOpacity={0.85}
                  >
                    {active && <View style={[StyleSheet.absoluteFillObject, { backgroundColor: plan.color + '0D', borderRadius: 18 }]} />}
                    ...
                    <View style={{ gap: 5 }}>
                      ...
                      {plan.locked.map(f => (
                        <View key={f} style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                          <Ionicons name="close" size={13} color={C.c35} />
              ...
              <View style={[s.infoCard, { backgroundColor: C.card, borderColor: C.border }]}>
                ...
                <Text style={{ flex: 1, fontSize: 12, color: C.c35, lineHeight: 18 }}>
                  You can upgrade, downgrade, or cancel anytime from your profile settings. No long-term commitment.
                </Text>
              </View>
            </View>
          )}
          ...
          {step === 4 && (
            <View style={{ gap: 14 }}>
              ...
              <View style={[s.previewCard, { borderColor: accent + '44' }]}>
                <View style={{ flexDirection: 'row', gap: 12, alignItems: 'flex-start', marginBottom: 10 }}>
                  <View style={[s.previewAv, { backgroundColor: accent + '22' }]}>
                ...
                <View style={{ gap: 4, paddingLeft: 4, marginBottom: 10 }}>
                  {specialties.length > 0 && (
                    <View style={{ flexDirection: 'row', gap: 6, alignItems: 'center' }}>
                      <Ionicons name="medkit-outline" size={12} color={C.c35} />
                      <Text style={s.previewRow}>{specialties.join(' · ')}</Text>
                    </View>
                  )}
                  {languages.length > 0 && (
                    <View style={{ flexDirection: 'row', gap: 6, alignItems: 'center' }}>
                      <Ionicons name="chatbubble-ellipses-outline" size={12} color={C.c35} />
                      <Text style={s.previewRow}>{languages.join(' · ')}</Text>
                    </View>
                  )}
                  {insurances.length > 0 && (
                    <View style={{ flexDirection: 'row', gap: 6, alignItems: 'center' }}>
                      <Ionicons name="shield-checkmark-outline" size={12} color={C.c35} />
                      <Text style={s.previewRow}>{insurances.join(' · ')}</Text>
                    </View>
                  )}
                  {location && (
                    <View style={{ flexDirection: 'row', gap: 6, alignItems: 'center' }}>
                      <Ionicons name="location-outline" size={12} color={C.c35} />
                      <Text style={s.previewRow}>{location}</Text>
                    </View>
                  )}
                  {availDays.length > 0 && (
                    <View style={{ flexDirection: 'row', gap: 6, alignItems: 'center' }}>
                      <Ionicons name="calendar-outline" size={12} color={C.c35} />
                      <Text style={s.previewRow}>Available: {availDays.join(', ')}</Text>
                    </View>
                  )}
                </View>
                {bio.length > 0 && (
                  <Text style={{ fontSize: 12, color: C.c60, lineHeight: 18 }} numberOfLines={3}>{bio}</Text>
                )}
              </View>
              ...
              <View style={s.summaryCard}>
                {[
                  ...
                  <View key={k} style={[s.summaryRow, i < 4 && { borderBottomWidth: 1, borderBottomColor: C.border }]}>
              ...
              <View style={[s.infoCard, { backgroundColor: C.card, borderColor: accent + '33' }]}>
                <Ionicons name="lock-closed-outline" size={18} color={accent} />
                <View style={{ flex: 1 }}>
                  <Text style={{ fontSize: 13, color: C.cream, fontWeight: '700', marginBottom: 3 }}>Verification</Text>
                  <Text style={{ fontSize: 12, color: C.c35, lineHeight: 18 }}>
                    We verify your NPI number and medical license before your profile goes live. This usually takes 24–48 hours.
                  </Text>
                </View>
              </View>
            </View>
        ...
        <View style={s.footer}>
          <TouchableOpacity
            style={[s.nextBtn, canNext[step] ? { backgroundColor: accent } : { backgroundColor: C.card, borderWidth: 1, borderColor: C.border }]}
            ...
            <Text style={[s.nextBtnTxt, { color: canNext[step] ? '#0D0F1A' : C.c35 }]}>
              {step < STEPS.length - 1
                ? step === 3
                  ? selectedPlan === 'free'
                    ? 'Continue with Free'
                    : `Start Free Trial — ${SUBSCRIPTION_PLANS.find(p => p.key === selectedPlan)?.name}`
                  : 'Continue'
                : 'Submit Profile'}
            </Text>
            <Ionicons
              ...
              size={16}
              color={canNext[step] ? '#0D0F1A' : C.c35}
            />
          </TouchableOpacity>
        </View>
...
const getStyles = (C) => StyleSheet.create({
  ...
  title:         { fontSize: 17, fontWeight: '800', color: C.cream, letterSpacing: -0.3 },
  sub:           { fontSize: 12, color: C.c35, marginTop: 1 },

  // Progress
  progressTrack: { height: 3, backgroundColor: C.border, marginHorizontal: 16, borderRadius: 2, marginBottom: 8 },
  progressFill:  { height: 3, borderRadius: 2 },

  // Steps
  stepTitle:     { fontSize: 22, fontWeight: '800', color: C.cream, marginBottom: 4, letterSpacing: -0.5 },
  ...
  chipTxt:       { fontSize: 12, color: C.c35, fontWeight: '600' },

  // Option row (consult mode)
  optionRow:     { flexDirection: 'row', alignItems: 'center', gap: 12, backgroundColor: C.card, borderWidth: 1, borderColor: C.border, borderRadius: 16, padding: 14, marginBottom: 8 },
  optionIconWrap:{ width: 38, height: 38, borderRadius: 12, alignItems: 'center', justifyContent: 'center' },
  ...
  dayTxt:        { fontSize: 12, color: C.c35, fontWeight: '600' },

  // Plans
  planCard:      { backgroundColor: C.card, borderWidth: 1, borderColor: C.border, borderRadius: 18, padding: 16, marginBottom: 12, overflow: 'hidden' },
  ...
  // Info card
  infoCard:      { flexDirection: 'row', alignItems: 'flex-start', gap: 10, borderWidth: 1, borderRadius: 14, padding: 14, marginTop: 4 },

  // Preview
  previewCard:   { backgroundColor: C.card, borderWidth: 1, borderRadius: 18, padding: 16 },
  previewAv:     { width: 52, height: 52, borderRadius: 16, alignItems: 'center', justifyContent: 'center' },
  previewBadge:  { flexDirection: 'row', alignItems: 'center', borderRadius: 8, paddingHorizontal: 7, paddingVertical: 3 },
  previewName:   { fontSize: 15, fontWeight: '800', color: C.cream, marginBottom: 2 },
  previewRow:    { fontSize: 12, color: C.c35 },

  // Summary
  summaryCard:   { backgroundColor: C.card, borderRadius: 16, borderWidth: 1, borderColor: C.border, overflow: 'hidden' },
  ...
  successIcon:   { width: 100, height: 100, borderRadius: 32, alignItems: 'center', justifyContent: 'center', marginBottom: 20, borderWidth: 1 },
  successTitle:  { fontSize: 28, fontWeight: '900', color: C.cream, marginBottom: 8, letterSpacing: -0.5 },
  successSub:    { fontSize: 14, color: C.c35, textAlign: 'center', lineHeight: 21, marginBottom: 24 },
  successCard:   { alignSelf: 'stretch', backgroundColor: C.card, borderWidth: 1, borderRadius: 18, padding: 16, marginBottom: 24, gap: 4 },
  previewName2:  { fontSize: 15, fontWeight: '800', color: C.cream },
  doneBtn:       { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, alignSelf: 'stretch', borderRadius: 16, paddingVertical: 15 },
